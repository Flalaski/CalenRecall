...
function numerologicalReduce(num) {
    while (num > 9) {
...
function calculateCrystalSides(entry) {
...
function GlobalTimelineMinimap({ selectedDate, viewMode, onTimePeriodSelect, onEntrySelect, minimapSize = 'medium', }) {
    const [entries, setEntries] = (0, react_1.useState)([]);
    const [timelineRangeKey, setTimelineRangeKey] = (0, react_1.useState)(0); // Track timeline range changes to trigger entry reloading
    const [isDragging, setIsDragging] = (0, react_1.useState)(false);
    const [mechanicalClick, setMechanicalClick] = (0, react_1.useState)(null);
    const [horizontalLocked, setHorizontalLocked] = (0, react_1.useState)(false);
    const [radialDial, setRadialDial] = (0, react_1.useState)(null);
    const [dragLimits, setDragLimits] = (0, react_1.useState)(null);
    const containerRef = (0, react_1.useRef)(null);
    const lastUpdateTimeRef = (0, react_1.useRef)(0);
    const dragStartPositionRef = (0, react_1.useRef)(null);
    const lastVerticalPositionRef = (0, react_1.useRef)(0);
    const scaleChangeLockRef = (0, react_1.useRef)(false);
    const verticalMovementAccumulatorRef = (0, react_1.useRef)(0);
    const initialMovementRef = (0, react_1.useRef)(null);
    const lastScaleChangeAccumulatorRef = (0, react_1.useRef)(0); // Track accumulator value at last scale change
    const deadZoneRef = (0, react_1.useRef)(0); // Dead zone threshold after scale change
    const lastBlipDateRef = (0, react_1.useRef)(null); // Track last date that triggered a micro blip
    const currentDragTargetDateRef = (0, react_1.useRef)(null); // Track the target date we're moving toward
        ...
        const loadEntries = async () => {
            try {
                ...
                if (window.electronAPI) {
                    ...
                    const selectedStr = (0, dateUtils_1.formatDate)(selectedDate);
    ...
    const timelineRangeRef = (0, react_1.useRef)(null);
    ...
    const calculateTimelineRange = (date, mode) => {
        let startDate;
        let endDate;
        switch (mode) {
            case 'decade': {
                const currentDecade = Math.floor((0, date_fns_1.getYear)(date) / 10) * 10;
                startDate = new Date(currentDecade - 50, 0, 1);
                endDate = new Date(currentDecade + 60, 11, 31);
                break;
            }
            case 'year': {
                const currentYear = (0, date_fns_1.getYear)(date);
                startDate = new Date(currentYear - 5, 0, 1);
                endDate = new Date(currentYear + 6, 11, 31);
                break;
            }
            case 'month': {
                const monthStart = (0, dateUtils_1.getMonthStart)(date);
                startDate = (0, date_fns_1.addMonths)(monthStart, -6);
                endDate = (0, date_fns_1.addMonths)(monthStart, 7);
                break;
            }
            case 'week': {
                const weekStart = (0, dateUtils_1.getWeekStart)(date);
                startDate = (0, date_fns_1.addWeeks)(weekStart, -8);
                endDate = (0, date_fns_1.addWeeks)(weekStart, 9);
                break;
            }
            case 'day': {
                const dayStart = new Date(date.getFullYear(), date.getMonth(), date.getDate());
                startDate = (0, date_fns_1.addDays)(dayStart, -14);
                endDate = (0, date_fns_1.addDays)(dayStart, 15);
                break;
            }
        }
    ...
    (0, react_1.useEffect)(() => {
        let shouldRecalculate = isInitialLoadRef.current ||
        ...
        if (!shouldRecalculate && timelineRangeRef.current) {
            const canonicalSelectedDate = (0, dateUtils_1.getCanonicalDate)(selectedDate, viewMode);
            const { startDate, endDate } = timelineRangeRef.current;
            // For decade view, compare decade boundaries
            if (viewMode === 'decade') {
                const selectedDecade = Math.floor((0, date_fns_1.getYear)(canonicalSelectedDate) / 10) * 10;
                const rangeStartDecade = Math.floor((0, date_fns_1.getYear)(startDate) / 10) * 10;
                const rangeEndDecade = Math.floor((0, date_fns_1.getYear)(endDate) / 10) * 10;
    ...
    const timelineData = (0, react_1.useMemo)(() => {
        ...
        switch (viewMode) {
            case 'decade': {
                ...
                const startDecade = Math.floor((0, date_fns_1.getYear)(startDate) / 10) * 10;
                const endDecade = Math.floor((0, date_fns_1.getYear)(endDate) / 10) * 10;
                for (let year = startDecade; year <= endDecade; year += 10) {
                    const decadeDate = new Date(year, 0, 1);
                    const isCurrent = Math.floor((0, date_fns_1.getYear)(selectedDate) / 10) * 10 === year;
                    segments.push({
                        date: decadeDate,
                        label: `${year}s`,
                        isCurrent,
            ...
            case 'year': {
                const startYear = (0, date_fns_1.getYear)(startDate);
                const endYear = (0, date_fns_1.getYear)(endDate);
                for (let year = startYear; year <= endYear; year++) {
                    const yearDate = new Date(year, 0, 1);
                    const isCurrent = (0, date_fns_1.getYear)(selectedDate) === year;
                    segments.push({
                        date: yearDate,
                        label: year.toString(),
                        isCurrent,
            ...
            case 'month': {
                ...
                let idx = 0;
                const monthStart = (0, dateUtils_1.getMonthStart)(selectedDate);
                while (current <= endDate) {
                    const isCurrent = (0, dateUtils_1.getMonthStart)(current).getTime() === monthStart.getTime();
                    segments.push({
                        date: new Date((0, dateUtils_1.getMonthStart)(current)),
                        label: (0, dateUtils_1.formatDate)(current, 'MMM yyyy'),
                        isCurrent,
                        viewMode: 'month',
                    });
                    if (isCurrent) {
            ...
            case 'week': {
                let current = new Date(startDate);
                let idx = 0;
                const weekStart = (0, dateUtils_1.getWeekStart)(selectedDate);
                while (current <= endDate) {
                    const weekStartDate = (0, dateUtils_1.getWeekStart)(current);
                    const isCurrent = weekStartDate.getTime() === weekStart.getTime();
                    segments.push({
                        date: weekStartDate,
                        label: (0, dateUtils_1.formatDate)(weekStartDate, 'MMM d'),
                        isCurrent,
                        viewMode: 'week',
                    });
                    if (isCurrent) {
            ...
            case 'day': {
                ...
                while (current <= endDate) {
                    ...
                    segments.push({
                        date: currentDay,
                        label: (0, dateUtils_1.formatDate)(currentDay, 'MMM d'),
                        isCurrent,
                        viewMode: 'day',
                    });
                    if (isCurrent) {
    ...
    const currentIndicatorMetrics = (0, react_1.useMemo)(() => {
        ...
        const periodStartDate = (0, dateUtils_1.getCanonicalDate)(selectedDate, viewMode);
        let periodEndDate;
        switch (viewMode) {
            case 'decade':
                periodEndDate = (0, dateUtils_1.getDecadeEnd)(selectedDate);
                break;
            case 'year':
                periodEndDate = (0, dateUtils_1.getYearEnd)(selectedDate);
                break;
            case 'month':
                periodEndDate = (0, dateUtils_1.getMonthEnd)(selectedDate);
                break;
            case 'week':
                periodEndDate = (0, dateUtils_1.getWeekEnd)(selectedDate);
                break;
            case 'day':
    ...
    const getSelectedSegmentColor = (0, react_1.useMemo)(() => {
        switch (viewMode) {
            case 'decade': {
                const decadeStart = Math.floor((0, date_fns_1.getYear)(selectedDate) / 10) * 10;
                return (0, dateUtils_1.getZodiacColorForDecade)(decadeStart) || '#9c27b0';
            }
            case 'year': {
                const yearDate = new Date((0, date_fns_1.getYear)(selectedDate), 0, 1);
                return (0, dateUtils_1.getZodiacColor)(yearDate) || '#0277bd';
            }
            case 'month': {
                const monthDate = new Date(selectedDate.getFullYear(), selectedDate.getMonth(), 15);
                return (0, dateUtils_1.getZodiacColor)(monthDate) || '#ef6c00';
            }
            case 'week': {
                // Use the middle of the week for color
                const weekStart = (0, dateUtils_1.getWeekStart)(selectedDate);
                const weekMiddle = (0, date_fns_1.addDays)(weekStart, 3);
                return (0, dateUtils_1.getZodiacColor)(weekMiddle) || '#2e7d32';
            }
            case 'day': {
                return (0, dateUtils_1.getZodiacColor)(selectedDate) || '#4a90e2';
            }
            default:
                return '#4a90e2';
        }
    ...
    const calculateMagnificationScale = (distanceFromIndicator, maxDistance = 50) => {
    ...
    const microIndicators = (0, react_1.useMemo)(() => {
        if (!timelineData.startDate || !timelineData.endDate) {
            return [];
        }
        const totalTime = timelineData.endDate.getTime() - timelineData.startDate.getTime();
        ...
        const timeScaleOrder = ['decade', 'year', 'month', 'week', 'day'];
        ...
        finerScales.forEach(scale => {
            ...
            const finerPeriodStart = (0, dateUtils_1.getCanonicalDate)(selectedDate, scale);
            let finerPeriodEnd;
            switch (scale) {
                case 'decade':
                    finerPeriodEnd = (0, dateUtils_1.getDecadeEnd)(selectedDate);
                    break;
                case 'year':
                    finerPeriodEnd = (0, dateUtils_1.getYearEnd)(selectedDate);
                    break;
                case 'month':
                    finerPeriodEnd = (0, dateUtils_1.getMonthEnd)(selectedDate);
                    break;
                case 'week':
                    finerPeriodEnd = (0, dateUtils_1.getWeekEnd)(selectedDate);
                    break;
                case 'day':
            ...
            switch (scale) {
                case 'year': {
                    ...
                    color = (0, dateUtils_1.getZodiacColor)(yearDate) || getViewModeColor(scale);
                    break;
                }
                case 'month': {
                    const monthDate = new Date(finerPeriodStart.getFullYear(), finerPeriodStart.getMonth(), 15);
                    color = (0, dateUtils_1.getZodiacColor)(monthDate) || getViewModeColor(scale);
                    break;
                }
                case 'week': {
                    // Use the middle of the week for color
                    const weekMiddle = (0, date_fns_1.addDays)(finerPeriodStart, 3);
                    color = (0, dateUtils_1.getZodiacColor)(weekMiddle) || getViewModeColor(scale);
                    break;
                }
                case 'day': {
                    color = (0, dateUtils_1.getZodiacColor)(finerPeriodStart) || getViewModeColor(scale);
                    break;
                }
                default:
    ...
    const centerX = (0, react_1.useMemo)(() => {
    ...
    const gradientStops = (0, react_1.useMemo)(() => {
    ...
    const minimapDimensions = (0, react_1.useMemo)(() => {
    ...
    const scaleYPositions = (0, react_1.useMemo)(() => {
    ...
    const separatorPositions = (0, react_1.useMemo)(() => {
    ...
    const buildInfinityBranchPath = (scale, direction) => {
    ...
    const generateFractalLightningBranch = (startX, startY, endX, endY, depth, maxDepth, entry, branchSeed, allowSplits = true, isEntryEnd = true // Track if endX/endY is the entry crystal
    ) => {
        // Validate inputs
        ...
        if (shouldSplit) {
            ...
            const rightBranch = generateFractalLightningBranch(branchX, branchY, rightX, rightY, depth + 1, maxDepth, entry, `${branchSeed}-R`, false, false // Side branch, not entry
            );
            const mainBranch = generateFractalLightningBranch(branchX, branchY, endX, endY, depth + 1, maxDepth, entry, `${branchSeed}-M`, false, isEntryEnd // Main branch to entry
            );
            ...
            const nextBranch = generateFractalLightningBranch(branchX, branchY, endX, endY, depth + 1, maxDepth, entry, branchSeed, false, isEntryEnd);
            return `L ${branchX},${branchY} ${nextBranch}`;
        }
    };
    // Optimized cosmic timeline flow - performance-focused
    // Always ends at entry crystal
    const generateCosmicTimelineFlow = (webNodeX, webNodeY, entryX, entryY, entry, levelIndex, simplified = false) => {
        // Validate inputs
        ...
        const timeFlow = [`M ${webNodeX},${webNodeY}`];
    ...
    // Always ends at entry crystal
    const generateChainLightning = (webNodeX, webNodeY, entryX, entryY, entry, levelIndex, simplified = false) => {
        // Validate inputs
        if (!isFinite(webNodeX) || !isFinite(webNodeY) || !isFinite(entryX) || !isFinite(entryY)) {
            return `M ${webNodeX},${webNodeY} L ${entryX},${entryY}`;
        }
        ...
        const lightning = [`M ${webNodeX},${webNodeY}`];
        const dx = entryX - webNodeX;
        const dy = entryY - webNodeY;
    ...
    const buildConnectionPath = (webNodeX, webNodeY, entryX, entryY, strategy, entry, levelIndex, viewportBounds, lodLevel) => {
        ...
        switch (strategy) {
            ...
            case 'spiral':
                ...
                const spiralPath = [`M ${webNodeX},${webNodeY}`];
    ...
    const isPointInViewport = (x, y, viewportBounds, margin = 50) => {
    ...
    const webNodes = (0, react_1.useMemo)(() => {
        const nodes = [];
        ...
        timeScaleOrder.forEach((scale, levelIndex) => {
            ...
            nodes.push({ x: leftBranchX, y: yPos, scale, level: levelIndex }, { x: rightBranchX, y: yPos, scale, level: levelIndex });
            ...
            if (levelIndex > 0) {
                ...
                if (isFinite(secondaryLeftX) && isFinite(secondaryRightX)) {
                    nodes.push({ x: secondaryLeftX, y: yPos, scale, level: levelIndex }, { x: secondaryRightX, y: yPos, scale, level: levelIndex });
                }
            }
            ...
            if (levelIndex > 1) {
                ...
                if (isFinite(tertiaryLeftX) && isFinite(tertiaryRightX)) {
                    nodes.push({ x: tertiaryLeftX, y: yPos, scale, level: levelIndex }, { x: tertiaryRightX, y: yPos, scale, level: levelIndex });
                }
            }
        });
    ...
    const allScaleMarkings = (0, react_1.useMemo)(() => {
        if (!timelineData.startDate || !timelineData.endDate) {
            return {
                decade: { major: [], minor: [] },
                year: { major: [], minor: [] },
        ...
        const scalesToCalculate = new Set([viewMode]);
        const scaleIndex = timeScaleOrder.indexOf(viewMode);
        if (scaleIndex > 0)
            scalesToCalculate.add(timeScaleOrder[scaleIndex - 1]); // Add scale above
        if (scaleIndex < timeScaleOrder.length - 1)
            scalesToCalculate.add(timeScaleOrder[scaleIndex + 1]); // Add scale below
        ...
        current = new Date((0, date_fns_1.getYear)(current), (0, date_fns_1.getMonth)(current), 1);
        let monthCount = 0;
        ...
        while (current <= endDate && monthCount < maxMonths) {
            ...
            const monthStart = new Date((0, date_fns_1.getYear)(current), (0, date_fns_1.getMonth)(current), 1);
            ...
            if (monthStart >= startDate && monthStart <= endDate && !monthSet.has(monthKey)) {
                ...
                const monthIndex = (0, date_fns_1.getMonth)(monthStart);
                ...
                if (isQuarterlyMonth) {
                    ...
                    scales.year.minor.push({
                        ...
                        label: (0, dateUtils_1.formatDate)(monthStart, 'MMM'),
                    });
                }
                monthCount++;
            }
        ...
        if (scalesToCalculate.has('month')) {
            ...
            while (current <= endDate && monthMajorCount < maxMonthMajors) {
                ...
                if (monthStart >= startDate && monthStart <= endDate && !monthMajorSet.has(monthKey)) {
                    ...
                    scales.month.major.push({
                        ...
                        label: (0, dateUtils_1.formatDate)(monthStart, 'MMM yyyy'),
                    });
                    monthMajorCount++;
                }
                current = (0, date_fns_1.addMonths)(current, 1);
            ...
            while (current <= endDate && monthWeekCount < 50) { // Limit weeks for month scale
                const weekStart = (0, dateUtils_1.getWeekStart)(current);
                ...
                if (weekStart >= startDate && weekStart <= endDate && !monthWeekSet.has(weekKey)) {
                    ...
                    scales.month.minor.push({
                        ...
                        label: (0, date_fns_1.getDate)(weekStart) <= 7 ? (0, dateUtils_1.formatDate)(weekStart, 'd') : undefined,
                    });
                    monthWeekCount++;
                }
                current = (0, date_fns_1.addWeeks)(current, 1);
            ...
            while (current <= endDate && weekCount < 100) { // Limit to 100 weeks
                ...
                if (weekStart >= startDate && weekStart <= endDate && !weekStartSet.has(weekKey)) {
                    ...
                    scales.week.major.push({
                        date: weekStart,
                        position: calculatePosition(weekStart),
                        label: (0, dateUtils_1.formatDate)(weekStart, 'MMM d'),
                    });
                    weekCount++;
                }
                current = (0, date_fns_1.addWeeks)(current, 1);
            ...
            while (current <= endDate && dayCount < maxDays) {
                ...
                if (!weekDaySet.has(dayKey)) {
                    ...
                    scales.week.minor.push({
                        ...
                        label: (0, date_fns_1.getDate)(current) % 7 === 1 ? (0, dateUtils_1.formatDate)(current, 'd') : undefined,
                    });
                    dayCount++;
                }
                current = (0, date_fns_1.addDays)(current, 1);
        ...
        if (scalesToCalculate.has('day')) {
            const dayMajorSet = new Set();
            current = new Date(startDate);
            let dayMajorCount = 0;
            const maxDayMajors = 100; // Limit day major marks
            while (current <= endDate && dayMajorCount < maxDayMajors) {
                const dayStart = new Date((0, date_fns_1.getYear)(current), (0, date_fns_1.getMonth)(current), (0, date_fns_1.getDate)(current));
                ...
                if (dayStart >= startDate && dayStart <= endDate && !dayMajorSet.has(dayKey)) {
                    ...
                    scales.day.major.push({
                        ...
                        label: (0, dateUtils_1.formatDate)(dayStart, 'MMM d'),
                    });
                    dayMajorCount++;
                }
                current = (0, date_fns_1.addDays)(current, 1);
    ...
    const hashToVerticalOffset = (str, maxOffset) => {
        let hash = 0;
    ...
    const entryPositions = (0, react_1.useMemo)(() => {
        ...
        filteredEntries.forEach(entry => {
            ...
            const entryDate = (0, dateUtils_1.getCanonicalDate)(rawEntryDate, entry.timeRange);
            ...
            clusterGroups.get(clusterKey).push({
                ...
                color: (0, entryColorUtils_1.calculateEntryColor)(entry),
            });
        });
    ...
    const focusWebNode = (0, react_1.useMemo)(() => {
    ...
    const memoryWebConnections = (0, react_1.useMemo)(() => {
        ...
        visibleEntries.forEach(({ entry, position, verticalOffset }) => {
            ...
            const strategies = ['direct', 'curved', 'hierarchical', 'web', 'spiral', 'organic'];
            ...
            if (sameScaleNodes.length > 0) {
                ...
                const connection = buildConnectionPath(nearestNode.x, nearestNode.y, entryX, entryY, primaryStrategy, entry, levelIndex, viewportBounds, lodLevel);
            ...
            if (shouldShowSecondary && otherScaleNodes.length > 0 && connections.length < maxConnectionsPerFrame) {
                ...
                const connection = buildConnectionPath(nearestOtherNode.x, nearestOtherNode.y, entryX, entryY, secondaryStrategy, entry, levelIndex, viewportBounds, lodLevel);
            ...
            if (lodLevel === 'high' && nodeDistances.length > 1 && connections.length < maxConnectionsPerFrame) {
                ...
                // Entry position is already validated, but ensure we're using current positions
                const connection = buildConnectionPath(tertiaryNode.x, tertiaryNode.y, entryX, entryY, tertiaryStrategy, entry, levelIndex, viewportBounds, lodLevel);
                // Skip if connection was culled or entry is off-screen
                if (connection && isPointInViewport(entryX, entryY, viewportBounds, 150)) {
            ...
            if (connections.length < maxConnectionsPerFrame && isPointInViewport(entryX, entryY, viewportBounds, 150)) {
                ...
                const connection = buildConnectionPath(focusWebNode.x, focusWebNode.y, entryX, entryY, focusStrategy, entry, levelIndex, viewportBounds, lodLevel);
                // Skip if connection was culled or entry is off-screen
                if (connection) {
        ...
        if ((lodLevel === 'high' || lodLevel === 'medium') && viewMode !== 'decade') {
            ...
            for (let i = 0; i < visibleEntries.length && entryConnectionCount < maxEntryConnections && connections.length < maxConnectionsPerFrame; i++) {
                ...
                for (let k = 0; k < connectionsToMake && entryConnectionCount < maxEntryConnections && connections.length < maxConnectionsPerFrame; k++) {
                    ...
                    const levelIndex = Math.max(timeScaleOrder.indexOf(sourceEntry.entry.timeRange), timeScaleOrder.indexOf(target.entry.timeRange));
                    // Create entry-to-entry connection
                    const connection = buildConnectionPath(sourceX, sourceY, target.position.x, target.position.y, entryStrategy, sourceEntry.entry, levelIndex, viewportBounds, lodLevel);
                    if (connection) {
    ...
    const handleMouseDown = (e) => {
        ...
        const audioContext = (0, audioUtils_1.getAudioContext)();
    ...
    const timelineDataRef = (0, react_1.useRef)(timelineData);
    const viewModeRef = (0, react_1.useRef)(viewMode);
    const selectedDateRef = (0, react_1.useRef)(selectedDate);
    const onTimePeriodSelectRef = (0, react_1.useRef)(onTimePeriodSelect);
    const horizontalLockedRef = (0, react_1.useRef)(horizontalLocked);
    const allScaleMarkingsRef = (0, react_1.useRef)(allScaleMarkings);
        ...
        const handleKeyDown = (e) => {
            ...
            if (isLeft || isRight) {
                ...
                switch (currentViewMode) {
                    case 'decade':
                        newDate = (0, date_fns_1.addYears)(currentSelectedDate, direction * 10);
                        break;
                    case 'year':
                        newDate = (0, date_fns_1.addYears)(currentSelectedDate, direction);
                        break;
                    case 'month':
                        newDate = (0, date_fns_1.addMonths)(currentSelectedDate, direction);
                        break;
                    case 'week':
                        newDate = (0, date_fns_1.addWeeks)(currentSelectedDate, direction);
                        break;
                    case 'day':
                        newDate = (0, date_fns_1.addDays)(currentSelectedDate, direction);
                        break;
                    default:
                        return;
                }
                // Play micro blip for date change
                (0, audioUtils_1.playMicroBlip)();
                currentOnTimePeriodSelect(newDate, currentViewMode);
            }
            else if (isUp || isDown) {
                // Change time scale (zoom in/out)
                const scaleOrder = ['decade', 'year', 'month', 'week', 'day'];
                ...
                if (isUp && currentIndex < scaleOrder.length - 1) {
                    ...
                    (0, audioUtils_1.playMechanicalClick)('up');
                    ...
                    const newViewMode = scaleOrder[currentIndex - 1];
                    (0, audioUtils_1.playMechanicalClick)('down');
    ...
    (0, react_1.useEffect)(() => {
        ...
        const handleWheel = (e) => {
            if (!containerRef.current || !timelineDataRef.current.startDate || !timelineDataRef.current.endDate)
                return;
            e.preventDefault();
            ...
            switch (currentViewMode) {
                case 'decade': {
                    ...
                    newDate = (0, date_fns_1.addMonths)(currentSelectedDate, (scrollDays > 0 ? 1 : -1) * yearsToScroll * 12);
                    break;
                }
                case 'year': {
                    // Scroll by about 1 month per scroll step
                    const monthsToScroll = Math.max(1, Math.round(Math.abs(scrollDays) / 30));
                    newDate = (0, date_fns_1.addMonths)(currentSelectedDate, (scrollDays > 0 ? 1 : -1) * monthsToScroll);
                    break;
                }
                case 'month': {
                    // Scroll by about 1 week per scroll step
                    const weeksToScroll = Math.max(1, Math.round(Math.abs(scrollDays) / 7));
                    newDate = (0, date_fns_1.addWeeks)(currentSelectedDate, (scrollDays > 0 ? 1 : -1) * weeksToScroll);
                    break;
                }
                case 'week': {
                    // Scroll by about 1 day per scroll step
                    const daysToScroll = Math.max(1, Math.round(Math.abs(scrollDays)));
                    newDate = (0, date_fns_1.addDays)(currentSelectedDate, (scrollDays > 0 ? 1 : -1) * daysToScroll);
                    break;
                }
                case 'day': {
                    // Scroll by 1 day per scroll step
                    newDate = (0, date_fns_1.addDays)(currentSelectedDate, scrollDays > 0 ? 1 : -1);
                    break;
                }
                default:
                    return;
    ...
    (0, react_1.useEffect)(() => {
        ...
        const handleMouseMoveGlobal = (e) => {
            ...
            const shouldLockHorizontal = (absVertical > 10 && absHorizontal < horizontalDeadZone) ||
    ...
    return (<div className="global-timeline-minimap">
      {/* Drag limits visualization - positioned relative to radial dial */}
      {isDragging && dragLimits && radialDial && (<div className="drag-limits-overlay" style={{
                left: `${radialDial.x}px`,
                top: `${radialDial.y}px`,
            }}>
          {/* Dead zone indicators - show when dead zone is active */}
          {lastScaleChangeAccumulatorRef.current !== 0 && (<>
              <div className="dead-zone-line dead-zone-top" style={{ top: `${dragLimits.deadZoneTop}px` }}/>
              <div className="dead-zone-line dead-zone-bottom" style={{ top: `${dragLimits.deadZoneBottom}px` }}/>
              {/* Dead zone fill */}
              {Math.abs(dragLimits.deadZoneBottom - dragLimits.deadZoneTop) > 0 && (<div className="dead-zone-fill" style={{
          ...
          <div className="vertical-threshold-line vertical-threshold-top" style={{ top: `${dragLimits.thresholdTop}px` }}>
            <span className="threshold-label">800px</span>
          </div>
          <div className="vertical-threshold-line vertical-threshold-bottom" style={{ top: `${dragLimits.thresholdBottom}px` }}>
            <span className="threshold-label">800px</span>
          </div>
          {/* Current position indicator */}
          {dragLimits.currentMovement !== undefined && (<>
              <div className="current-movement-indicator" style={{ top: `${-dragLimits.currentMovement}px` }}>
              ...
              <div className="compact-scale-indicator">
                ...
                {lastScaleChangeAccumulatorRef.current !== 0 && (<>
                    <div className="scale-line scale-deadzone-top" style={{ top: `${dragLimits.deadZoneTop}px` }}>
                      <span className="scale-label">Dead Zone</span>
                    </div>
                    <div className="scale-line scale-deadzone-bottom" style={{ top: `${dragLimits.deadZoneBottom}px` }}>
                      <span className="scale-label">Dead Zone</span>
                    </div>
                  </>)}
                <div className="scale-line scale-threshold-top" style={{ top: `${dragLimits.thresholdTop}px` }}>
                  <span className="scale-label">800px</span>
                </div>
                <div className="scale-line scale-threshold-bottom" style={{ top: `${dragLimits.thresholdBottom}px` }}>
                  <span className="scale-label">800px</span>
                </div>
              </div>
            </>)}
      ...
      {radialDial && isDragging && (<div className="radial-dial" style={{
                left: `${radialDial.x}px`,
                top: `${radialDial.y}px`,
            }}>
      ...
      {mechanicalClick && (<div className={`mechanical-click-feedback ${mechanicalClick.direction}`}>
      ...
      <div ref={containerRef} className={`minimap-container minimap-size-${minimapSize} ${isDragging ? 'dragging' : ''} ${mechanicalClick ? 'mechanical-click-active' : ''} ${horizontalLocked ? 'horizontal-locked' : ''}`} onMouseDown={handleMouseDown}>
        ...
        <svg className="fractal-web" viewBox={`0 0 1000 ${minimapDimensions.height}`} preserveAspectRatio="none" shapeRendering="crispEdges" style={{ imageRendering: 'crisp-edges' }}>
          <defs>
          ...
          {timeScaleOrder.map((range, idx) => {
            ...
            return (<rect key={`section-band-${range}`} x="0" y={sectionTop} width="1000" height={sectionHeight} fill={sectionColor} opacity={opacity} style={{
          ...
          <line x1={Math.round(centerX) + 0.5} y1="0.5" x2={Math.round(centerX) + 0.5} y2={`${minimapDimensions.height - 0.5}`} stroke="#4a90e2" strokeWidth="2" opacity="0.55" className="infinity-tree-trunk" shapeRendering="crispEdges"/>
          ...
          {separatorPositions.map((yPos, idx) => {
            ...
            return (<g key={`separator-${idx}`}>
                {/* Base line with gradient - spans full width */}
                <line x1="0" y1={yPosFormatted} x2="1000" y2={yPosFormatted} stroke={`url(#${gradientId})`} strokeWidth="1" shapeRendering="crispEdges"/>
                {/* Thicker center overlay for light-catching effect - spans full width */}
                <line x1="0" y1={yPosFormatted} x2="1000" y2={yPosFormatted} stroke={activeColor} strokeWidth="2" opacity="0.8" shapeRendering="crispEdges" style={{
          ...
          <g className="infinity-tree">
            ...
            return (<g key={scale}>
                  ...
                  <path d={buildInfinityBranchPath(scale, 'left')} stroke={glowColorPrimary} strokeWidth={thickness} fill="none" opacity={baseOpacity} className={`infinity-tree-branch ${isActiveScale ? 'active' : ''}`}/>
                  <path d={buildInfinityBranchPath(scale, 'right')} stroke={glowColorSecondary} strokeWidth={thickness - 0.4} fill="none" opacity={baseOpacity * 0.8} className={`infinity-tree-branch secondary ${isActiveScale ? 'active' : ''}`}/>

                  {/* Subtle crystalline filaments hugging the scale band */}
                  <path d={`M ${centerX - 12 * (levelIndex + 1)},${scaleYPositions[scale]} 
                        L ${centerX + 12 * (levelIndex + 1)},${scaleYPositions[scale]}`} stroke={glowColorSecondary} strokeWidth="0.8" opacity={baseOpacity * 0.4} className="infinity-tree-filament"/>
                </g>);
        })}
          </g>
          ...
          <g className="memory-web-connections">
            ...
            if (connectionType === 'entry-to-entry' && targetEntry) {
                ...
                const sourceColor = (0, entryColorUtils_1.calculateEntryColor)(entry);
                const targetColor = (0, entryColorUtils_1.calculateEntryColor)(targetEntry);
                ...
                connectionColor = rgbToHex((sourceRgb.r + targetRgb.r) / 2, (sourceRgb.g + targetRgb.g) / 2, (sourceRgb.b + targetRgb.b) / 2);
            }
                ...
                connectionColor = rgbToHex((entryRgb.r * 0.6 + focusRgb.r * 0.4), (entryRgb.g * 0.6 + focusRgb.g * 0.4), (entryRgb.b * 0.6 + focusRgb.b * 0.4));
            }
            else {
                // Web connections use entry color
                connectionColor = (0, entryColorUtils_1.calculateEntryColor)(entry);
            }
            ...
            return (<g key={connectionKey}>
                  {/* Connection path with adaptive styling */}
                  {/* Ensure path ends exactly at entry crystal center by using precise coordinates */}
                  <path d={connection.path} stroke={connectionColor} strokeWidth={connection.strokeWidth} fill="none" opacity={connection.opacity} strokeDasharray={dashArray} strokeLinecap="round" strokeLinejoin="round" className={`${connectionClass} ${connectionTypeClass}`} style={{
                    filter: `drop-shadow(0 0 ${connection.strokeWidth * 2}px ${connectionColor})`,
                }} shapeRendering="geometricPrecision"/>
                  
                  {/* Connection glow layer for depth */}
                  <path d={connection.path} stroke={connectionColor} strokeWidth={connection.strokeWidth * 1.5} fill="none" opacity={connection.opacity * 0.2} strokeDasharray={dashArray} strokeLinecap="round" strokeLinejoin="round" className={`${connectionClass}-glow ${connectionTypeClass}`}/>
                </g>);
        })}
          </g>
          ...
          {allScaleMarkings.decade.major
            ...
            return (<line key={`decade-major-${idx}`} x1={x} y1="0.5" x2={x} y2="40.5" stroke={isCurrentScale ? "#4a90e2" : "#333"} strokeWidth={isCurrentScale ? "4" : "3"} opacity={isCurrentScale ? "0.9" : "0.8"} shapeRendering="crispEdges"/>);
        })}
          {allScaleMarkings.decade.minor
            ...
            const x = Math.round((mark.position / 100) * 1000) + 0.5;
            const isCurrentScale = viewMode === 'decade';
            return (<line key={`decade-minor-${idx}`} x1={x} y1="0.5" x2={x} y2="30.5" stroke={isCurrentScale ? "#6ab7ff" : "#555"} strokeWidth={isCurrentScale ? "3" : "2"} opacity={isCurrentScale ? "0.7" : "0.6"} shapeRendering="crispEdges"/>);
        })}
          ...
          {allScaleMarkings.year.major
            ...
            return (<line key={`year-major-${idx}`} x1={x} y1="40.5" x2={x} y2="80.5" stroke={isCurrentScale ? "#4a90e2" : "#444"} strokeWidth={isCurrentScale ? "4" : "3"} opacity={isCurrentScale ? "0.85" : "0.75"} shapeRendering="crispEdges"/>);
        })}
          {allScaleMarkings.year.minor
            ...
            const x = Math.round((mark.position / 100) * 1000) + 0.5;
            const isCurrentScale = viewMode === 'year';
            return (<line key={`year-minor-${idx}`} x1={x} y1="40.5" x2={x} y2="70.5" stroke={isCurrentScale ? "#6ab7ff" : "#666"} strokeWidth={isCurrentScale ? "2" : "2"} opacity={isCurrentScale ? "0.6" : "0.5"} shapeRendering="crispEdges"/>);
        })}
          ...
          {allScaleMarkings.month.major
            ...
            return (<line key={`month-major-${idx}`} x1={x} y1="80.5" x2={x} y2="120.5" stroke={isCurrentScale ? "#4a90e2" : "#555"} strokeWidth={isCurrentScale ? "4" : "3"} opacity={isCurrentScale ? "0.9" : "0.7"} shapeRendering="crispEdges"/>);
        })}
          {allScaleMarkings.month.minor
            ...
            const x = Math.round((mark.position / 100) * 1000) + 0.5;
            const isCurrentScale = viewMode === 'month';
            return (<line key={`month-minor-${idx}`} x1={x} y1="80.5" x2={x} y2="110.5" stroke={isCurrentScale ? "#6ab7ff" : "#777"} strokeWidth={isCurrentScale ? "2" : "1"} opacity={isCurrentScale ? "0.7" : "0.5"} shapeRendering="crispEdges"/>);
        })}
          ...
          {allScaleMarkings.week.major
            ...
            return (<line key={`week-major-${idx}`} x1={x} y1="120.5" x2={x} y2="160.5" stroke={isCurrentScale ? "#4a90e2" : "#555"} strokeWidth={isCurrentScale ? "3" : "3"} opacity={isCurrentScale ? "0.8" : "0.6"} shapeRendering="crispEdges"/>);
        })}
          {allScaleMarkings.week.minor
            ...
            const x = Math.round((mark.position / 100) * 1000) + 0.5;
            const isCurrentScale = viewMode === 'week';
            return (<line key={`week-minor-${idx}`} x1={x} y1="120.5" x2={x} y2="150.5" stroke={isCurrentScale ? "#6ab7ff" : "#888"} strokeWidth={isCurrentScale ? "1" : "1"} opacity={isCurrentScale ? "0.6" : "0.4"} shapeRendering="crispEdges"/>);
        })}
          ...
          {allScaleMarkings.day.major
            ...
            return (<line key={`day-major-${idx}`} x1={x} y1="160.5" x2={x} y2="200.5" stroke={isCurrentScale ? "#4a90e2" : "#666"} strokeWidth={isCurrentScale ? "3" : "2"} opacity={isCurrentScale ? "0.7" : "0.5"} shapeRendering="crispEdges"/>);
        })}
          {allScaleMarkings.day.minor
            .filter(mark => Math.abs(mark.position - currentIndicatorMetrics.position) <= LOCALIZATION_RANGE)
            .map((mark, idx) => {
            const x = Math.round((mark.position / 100) * 1000) + 0.5;
            return (<line key={`day-minor-${idx}`} x1={x} y1="160.5" x2={x} y2="190.5" stroke="#999" strokeWidth="1" opacity="0.3" shapeRendering="crispEdges"/>);
        })}
        </svg>
        ...
        <div className="scale-labels">
          ...
          {allScaleMarkings.decade.major
            ...
            if (!mark.label)
                return false;
            if (!isFinite(mark.position))
                return false;
            if (mark.position < 0 || mark.position > 100)
                return false;
            return true;
        })
            .map((mark, idx) => {
            ...
            const zodiacColor = (0, dateUtils_1.getZodiacColorForDecade)(decadeStart) || '#4a90e2';
            ...
            return (<div key={decadeUniqueKey} className={`scale-label decade-label ${viewMode === 'decade' ? 'current-scale' : ''}`} style={decadeLabelStyle} data-position={labelPosition} data-label={mark.label}>
                  {mark.label}
                </div>);
        })}
          ...
          {allScaleMarkings.year.major
            ...
            const zodiacColor = (0, dateUtils_1.getZodiacColor)(yearDate);
            // Calculate opacity and magnification based on distance from indicator
            const indicatorPosition = isFinite(currentIndicatorMetrics.position)
                ? Number(currentIndicatorMetrics.position)
                : 50; // Default to center if invalid
            ...
            return (<div key={yearUniqueKey} className={`scale-label year-label ${viewMode === 'year' ? 'current-scale' : ''}`} style={yearLabelStyle} data-position={labelPosition} data-label={mark.label}>
                  {mark.label}
                </div>);
        })}
          {allScaleMarkings.year.minor
            ...
            const zodiacColor = (0, dateUtils_1.getZodiacColor)(monthDate);
            // Build the style object explicitly to ensure all properties are set correctly
            ...
            if (labelPosition < 5 && idx < 10) {
                console.log(`[DEBUG] Year minor label cluster check: "${mark.label}" at position ${labelPosition}%, date: ${mark.date ? (0, dateUtils_1.formatDate)(mark.date) : 'no date'}, style left: ${labelStyle.left}`);
            }
            return (<div key={uniqueKey} className="scale-label year-minor-label" style={labelStyle} data-position={labelPosition} data-label={mark.label} data-debug={`pos:${labelPosition},opacity:${labelOpacity.toFixed(2)}`}>
                  {mark.label}
                </div>);
        })}
          ...
          {allScaleMarkings.month.major
            ...
            return mark.label && (<div key={`month-label-${idx}`} className={`scale-label month-label ${viewMode === 'month' ? 'current-scale' : ''}`} style={{
                    left: `${mark.position}%`,
                    top: '85px',
                    color: zodiacColor,
                    fontSize: fontSize,
          ...
          {allScaleMarkings.month.minor
            ...
            const calculatedOpacity = 1 - (absDistance / LOCALIZATION_RANGE);
            const labelOpacity = Math.max(0.2, Math.min(1, calculatedOpacity));
            return mark.label && (<div key={`month-minor-label-${idx}`} className="scale-label month-minor-label" style={{
                    left: `${mark.position}%`,
                    top: '88px',
                    color: zodiacColor,
                    fontSize: fontSize,
          ...
          {allScaleMarkings.week.major
            ...
            const calculatedOpacity = 1 - (absDistance / LOCALIZATION_RANGE);
            const labelOpacity = Math.max(0.2, Math.min(1, calculatedOpacity));
            return mark.label && (<div key={`week-label-${idx}`} className={`scale-label week-label ${viewMode === 'week' ? 'current-scale' : ''}`} style={{
                    left: `${mark.position}%`,
                    top: '125px',
                    fontSize: fontSize,
                    opacity: labelOpacity,
          ...
          {allScaleMarkings.day.major
            ...
            const calculatedOpacity = 1 - (absDistance / LOCALIZATION_RANGE);
            const labelOpacity = Math.max(0.2, Math.min(1, calculatedOpacity));
            return mark.label && (<div key={`day-label-${idx}`} className={`scale-label day-label ${viewMode === 'day' ? 'current-scale' : ''}`} style={{
                    left: `${mark.position}%`,
                    top: '165px',
                    fontSize: fontSize,
                    opacity: labelOpacity,
        ...
        <div className="timeline-segments">
          {timelineData.segments.map((segment, idx) => {
            ...
            return (<div key={segmentKey} className={`timeline-segment ${segment.isCurrent ? 'current' : ''} ${isNearCurrent ? 'near-current' : ''}`} style={{ left: `${position}%` }} onClick={() => onTimePeriodSelect(segment.date, segment.viewMode)} title={(0, dateUtils_1.formatDate)(segment.date, 'MMM d, yyyy')}>
                <div className="segment-indicator"/>
                {segment.isCurrent && (<div className="segment-label">{segment.label}</div>)}
              </div>);
        })}
        </div>
        ...
        <div className="entry-indicators">
          {entryPositions.map(({ entry, position, color, clusterIndex, clusterSize, clusterAngle, polygonClipPath, sides }, idx) => {
            ...
            return (<div key={entry.id || idx} className={`entry-indicator-wrapper ${isInCluster ? 'in-cluster' : ''} ${isFocusedSection ? 'focused-section' : ''}`} style={{
                    left: `${position}%`,
                    top: isInCluster
                ...
                }} onClick={handleClick} title={`${entry.title} (${entry.timeRange})${isInCluster ? ` - ${clusterSize} entries` : ''}`} data-cluster-size={isInCluster ? clusterSize : undefined} data-cluster-index={isInCluster ? clusterIndex : undefined} onMouseEnter={(e) => {
                    if (isInCluster) {
        ...
        {timelineData.segments.length > 0 && (<div key="current-indicator" className="current-period-indicator" style={{
            ...
            {microIndicators.map((indicator, idx) => (<div key={`micro-indicator-${indicator.scale}-${idx}`} className={`micro-indicator micro-indicator-${indicator.scale}`} style={{
                    left: `${indicator.left}%`,
